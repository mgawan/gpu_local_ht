...
//calls the kernels, revcomps, copy backs walks and
    ...
    for(int slice = 0; slice < iterations; slice++){
        ...
        unsigned total_threads = vec_size*WARP_SIZE;// we need one warp (32 threads) per extension, vec_size = extensions
        ...
        print_vals("Timing Iterative Walks Kernel...");
        hipEvent_t start, stop;
        hipEventCreate(&start);
        hipEventCreate(&stop);

        hipEventRecord(start);
        hipLaunchKernelGGL(iterative_walks_kernel, dim3(blocks), dim3(thread_per_blk), 0, 0, cid_d, ctg_seq_offsets_d, ctg_seqs_d, reads_right_d, quals_right_d, reads_r_offset_d, rds_r_cnt_offset_d, 
       ...
       // rev_comp_.timer_start();
        ...
        print_vals("Timing Iterative Walks Kernel...");
        hipEventRecord(start);
        hipLaunchKernelGGL(iterative_walks_kernel, dim3(blocks), dim3(thread_per_blk), 0, 0, cid_d, ctg_seq_offsets_d, ctg_seqs_d, reads_left_d, quals_left_d, reads_l_offset_d, rds_l_cnt_offset_d, 
        depth_d, d_ht, prefix_ht_size_d, d_ht_bool, mer_len, max_mer_len, term_counts_d, num_walks, max_walk_len, sum_ext, max_read_size, max_read_count, qual_offset, longest_walks_d, mer_walk_temp_d, final_walk_lens_d, vec_size);
        hipEventRecord(stop);
