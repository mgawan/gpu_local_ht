...
void call_kernel(std::vector<CtgWithReads>& data_in, uint32_t max_ctg_size, uint32_t total_r_reads, uint32_t total_l_reads, uint32_t max_read_size, uint32_t max_r_count, uint32_t max_l_count, int mer_len,int max_reads_count);
...
int main (int argc, char* argv[]){
   ...
   // std::string out_file = argv[2];
    ...
    call_kernel(mid_slice, mid_max_contig_sz, mids_tot_r_reads, mids_tot_l_reads, max_read_size, mid_r_max, mid_l_max, max_mer_len,max_reads_count);
    print_vals("midsup calling",  "mids count:", midsup_slice.size());
    max_reads_count = 100;
    call_kernel(midsup_slice, midsup_max_contig_sz, midsup_tot_r_reads, midsup_tot_l_reads, max_read_size, midsup_r_max, midsup_l_max, max_mer_len,max_reads_count);

    print_vals("outliers calling", "outliers count:", outlier_slice.size());
     max_reads_count = 239;
  //  overall_time.timer_start();
    call_kernel(outlier_slice, outliers_max_contig_sz, outliers_tot_r_reads, outliers_tot_l_reads, max_read_size, outlier_r_max, outlier_l_max, max_mer_len, max_reads_count);
...
//allocating maximum possible memory for a single iteration
    ...
    uint32_t *rds_l_cnt_offset_d, *rds_r_cnt_offset_d;
